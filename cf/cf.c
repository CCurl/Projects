...
CELL rpop() { return rstk[rsp--]; }

int charAvailable() { return qKey(); }
int getChar() { return key(); }
...
void run(ushort pc) {
    ...
    case ';': t1 = rpop(); if (!t1) { rsp = 0; return; } pc = t1;   NEXT;
...
void doOuter(char* cp) {
    ...
    while (1) {
        char c = peekChar();
        if (!c) { return; }
        if (betw(c, 1, 7)) { mode = nextChar(); continue; }
