...
static int x = 0, i, c, v, m, r, b = 8000, p, n, s, f, ro = 64;
...
void f100() /* d */ { u=st.b[p++]; st.i[u+ro]--; }
/* e-h */ void f101() {} void f102() {} void f103() {} void f104() {} /* e-h */
void f105() /* i */ { u=st.b[p++]; st.i[u+ ro]++; }
/* j-m */ void f106() {} void f107() {} void f108() {} void f109() {} /* j-m */
void f110() /* n */ { putc('\n',stdout); }
/* o-q */ void f111() {} void f112() {} void f113() {}
void f114() /* r */ { u = st.b[p++]; push(st.i[u+ ro]); }
void f115() /* s */ { u = st.b[p++]; st.i[u+ ro] = pop(); }
...
void R(int sb) { s=(s<1)?0:s; p=sb; r=35; while (st.b[p]) { u = st.b[p++]; q[u](); } }
void L() { v = m; printf("\ns2>"); fgets(&st.b[v], 99, stdin); R(v); }
void main(int argc, char* argv[]) {
    m = b; for (i = 0; i < 2500; i++) { st.i[i] = 0; }
    if (argc > 1) {
        FILE* fp = fopen(argv[1], "rt"); if (fp) {
            while ((c = fgetc(fp)) != EOF) { if (btw(c,32,126)) { st.b[m++]=c; } };
        fclose(fp); R(b); } else { printf("file?"); }
    }
    while (1) { L(); };
}
