...
void dumpOp(op_t *op) {
    printf("\npc: %d: ", pc-1);
    printf("[0:%d%c]", OPC(0), BTW(OPC(0),32,126) ? (char)OPC(0) : '.');
    for (int i=1; i<CELL_SZ; i++) { printf("[%d,%02d]", i, OPC(i)); }
}
...
void runReg(int start) {
    ...
    switch (OPC(0)) {
        ...
        NCASE2 'l': PS(loc[locBase+OPC(1)]);
        NCASE2 'L': loc[locBase+OPC(1)] = PP;
        NCASE2 'm': reg[OPC(1)] = OP32(1);
        NCASE2 'M': loc[locBase+OPC(1)] = OP32(1);
        NCASE2 'n': PS(OP32(1));
            // printf("-n[%d]-", OP32(1));
        NCASE2 'r': PS(reg[OPC(1)]);
            // printf("-r[%d]-", OP32(1));
        NEXT2; default: dumpOp(op); NEXT2;
    }
}
...
void pw() {
    ...
    if (BTW(w[0],'0','9')) {       // 1234 or 1234sX
        ...
        if ((w[l]=='s') && BTW(w[l+1],'0','9')) { c01_l1('M', w[l+1], PP32); }
        else { c0_l1('n', PP32); }
        return;
    }
    ...
    switch (w[0]) {
        ...
        RCASE '*': c0123( w[0], w[1], w[2], w[3]);
        RCASE '-': c0123( w[0], w[1], w[2], w[3]);
        RCASE '/': c0123( w[0], w[1], w[2], w[3]);
        RCASE 'r': if (!doRegR()) { doId(); }        // RegRead
        RCASE 's': if (!doRegS()) { doId(); }        // RegSet
        RCASE 't': c01( w[0], w[1]);    // Timer
        RCASE '.': c01( w[0], w[1] );
        RCASE ',': c01( w[0], w[1] );
        RCASE ']': c0(  w[0] );
        RCASE '[': c0(  w[0] );
